...
def booleval(target, pts = 1, level = 1):
    ...
    title = 'Boolean logic'
    question = 'What is the value of ${:s}$ with precedence (from highest to lowest) $ {:s} $ and the assignment {:s}?'.format(expr, lp, val)
    ...
    expr = expr.replace('a', '1' if a == 'top' else '0')
    expr = expr.replace('b', '1' if b == 'top' else '0')
    expr = expr.replace('c', '1' if c == 'top' else '0')
    expr = expr.replace('d', '1' if d == 'top' else '0')
...
def modulo(target, pts = 1, level = 1): 
    ...
    hint = '''Recall that the modulo operator refers to the residue of an integer division.
...
def vertexdegree(target, count = 3, level = 3):
    ...
    title = 'Vertex degrees in a simple, undirected graph'
